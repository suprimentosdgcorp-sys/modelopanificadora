...
function renderVendas(){
  ...
  ev.innerHTML = ''
    ...
    +'<div class="kpi" style="--kt:var(--g2)"><div class="kpi-label">Faturamento Total</div><div class="kpi-val">'+fmt(totFat)+'</div><div class="kpi-sub">Meta: '+fmt(metaM)+'</div></div>'
...
function renderConsolidado(){
  ...
  lojas.forEach(function(l){
    ...
    T[l].pct = T[l].meta>0?T[l].rec/T[l].meta:0; // meta não considera vendas extras
    T[l].rec += getExtrasTotal(l);
    T[l].despEscrit = getDesp(l).reduce(function(s,x){return s+(x.val||0);},0);
    T[l].totDesp = T[l].desp + T[l].despEscrit;
    T[l].lucro = T[l].rec - T[l].totDesp;
...
function getDesemp2026(loja){
  ...
  var meses={};
  function ensure(m){ if(!meses[m]) meses[m]={mes:m,rec:0,escrit:0,caixa:0}; return meses[m]; }
  ...
  return ORDEM.filter(function(m){return meses[m];}).map(function(m){return meses[m];});
}
...
function renderCompChart(){
  ...
  function get(h,yr){
    var rec=yr===25?h.rec25:h.rec26, desp=yr===25?h.desp25:h.desp26;
