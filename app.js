...
var ABAS = {
  frei:    {v:'VD FREI',          d:'DESPESAS FREI',        cx:'DESPESA DE CAIXA',  w:'WHATS FREI',      i:'IFOOD FREI',      ve:'VENDAS EXTRAS FREI',   de:'DESP. EXTRAS FREI',
            ...
            metaTeto:42000, metaWhats:0, metaLucro:0.22, meta:357103, metaDia:17000, cor:'#B8860B'},
  eliseu:  {v:'VD ELISEU',        d:'DESPESAS ELISEU',      cx:'DESPESA DE CAIXA',  w:'WHATS ELISEU',    i:'IFOOD ELISEU',    ve:'VENDAS EXTRAS ELISEU',  de:'DESP. EXTRAS ELISEU',
            ...
            metaTeto:40000, metaWhats:0, metaLucro:0.21, meta:216871, metaDia:12000, cor:'#1E6B3A'},
  petronio:{v:'VD UNIVERSIT',     d:'DESPESAS UNIVERSIT',   cx:'DESPESA DE CAIXA',  w:'WHATS UNIVERSIT', i:'IFOOD UNIVERSIT', ve:'VENDAS EXTRAS UNIVERSIT',de:'DESP. EXTRAS UNIVERSIT',
            ...
            metaTeto:20000, metaWhats:0, metaLucro:0.22, meta:295668, metaDia:12000, cor:'#6B1A1A'},
};
...
function parseWhats(rows){
  ...
  function toMes(ds){var p=String(ds).split('/');var ns=['','JAN','FEV','MAR','ABR','MAI','JUN','JUL','AGO','SET','OUT','NOV','DEZ'];return(ns[parseInt(p[1])]||'')+'/'+p[2];}
  var reData=/^\d{2}[\/\-]\d{2}/;
  return rows.filter(function(r){return reData.test(String(gf(r,['DATA'])||'').trim());})
    .map(function(r){
      ...
      return {
        ...
        pix:   toNum(gf(r,['PIX (R$)','PIX\n(R$)','PIX'])),
        cartao:toNum(gf(r,['CARTÃO (R$)','CARTÃO\n(R$)','CARTAO (R$)','CARTÃO'])),
        especie:toNum(gf(r,['ESPÉCIE (R$)','ESPÉCIE\n(R$)','ESPECIE (R$)'])),
        total: toNum(gf(r,['TOTAL (R$)','TOTAL\n(R$)','TOTAL'])),
      };
    });
}
...
function carregarTudo(){
  ...
  function carregarLoja(l){
    return Promise.all([
      ...
      fetchSheet(ABAS[l].cx).then(parseDespCaixa).catch(function(e){console.warn('[API] despCx '+l+' falhou:',e);return[];})
    ]);
  }
  ...
  function processarLoja(l,res){
    ...
    STATE[l].despCx= [];  // desp caixa agora vem dentro de VENDAS (campo DESP CAIXA)
    STATE[l].whats = whats;
    STATE[l].ifood = ifood;
  }
