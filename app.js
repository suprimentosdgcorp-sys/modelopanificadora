...
// ═══════════════════════════════════════════════════════════════════════
var SHEETDB = 'https://sheetdb.io/api/v1/2zaqzajss2dni';
...
var ABAS = {
  ...
  petronio:{v:'VD - UNIVERSITÁRIA',d:'DESPESAS - UNIVERSITÁRIA', cx:'DESP CAIXA - UNIVERSITÁRIA', label:'Universitária',
...
function parseDespCaixa(rows){
  ...
  return rows.filter(function(r){
    ...
    return {
      ...
      val:toNum(gf(r,['VALOR (R$)','VALOR','valor'])),
      forma:gf(r,['FORMA DE PAGAMENTO','FORMA DE\nPAGAMENTO','forma de pagamento']),
...
function carregarTudo(){
  ...
  function processarLoja(l,res){
    ...
    var desp  =res[1]&&res[1].length?res[1]:null;
    var despCx=res[2]&&res[2].length?res[2]:[];
    if(!dias){usandoDemo=true;console.warn('[Dashboard] '+l+': sem dados – usando amostra');}
    else console.log('[Dashboard] '+l+': '+dias.length+' dias, meses:',[...new Set(dias.map(function(d){return d.mes;}))].join(', '));
    STATE[l].dias    =dias||getSampleDias(l);
    STATE[l].desp    =desp||getSampleDesp(l);
    STATE[l].despDemo=!desp;
    STATE[l].despCx  =despCx||[];
  }

  fetchSheet('METAS').then(parseMetas).catch(function(){}).finally(function(){
    carregarLoja('frei')
      .then(function(res){processarLoja('frei',res);return delay(600);})
      .then(function(){return carregarLoja('eliseu');})
      .then(function(res){processarLoja('eliseu',res);return delay(600);})
      .then(function(){return carregarLoja('petronio');})
      .then(function(res){processarLoja('petronio',res);})
      .catch(function(e){
        console.error('[Dashboard] Erro geral:',e);
        lojas.forEach(function(l){STATE[l].dias=getSampleDias(l);STATE[l].desp=getSampleDesp(l);STATE[l].despDemo=true;});
      })
      .finally(function(){
...
function renderVendas(){
  ...
  ev.innerHTML = ''
    ...
    +'% meta mensal: <strong style="color:'+(pct>=1?'var(--grn)':'var(--red)')+'">'+fmtP(pct)+'</strong></div></div>'
    ...
    +'<div class="kpi" style="--kt:'+(L.pctTeto>=1?'var(--red)':L.pctTeto>=0.8?'var(--amb)':'var(--grn)')+'"><div class="kpi-label">Desp. Caixa</div><div class="kpi-val" style="color:'+(L.pctTeto>=1?'var(--red)':L.pctTeto>=0.8?'var(--amb)':'var(--grn)')+'">'+fmt(L.totDespCx)+'</div><div class="kpi-sub">'+(L.teto>0?fmtP(L.pctTeto)+' do teto':'Sem teto')+'</div></div>'
    +lucroKpi
  ...
  CHARTS['cv-d'] = new Chart(el('cv-d'),{
    ...
    data:{
      ...
      datasets:[
        ...
        {label:'Meta',data:allDias.map(function(d){return d.metaDia>0?d.metaDia:null;}),type:'line',
