...
function carregarTudo(fresh){
  ...
  function processarLoja(l,res){
    ...
    // parseVendas() não traz "whats" (aba WHATS é separada) — mescla por data
    // para que o Diário de Vendas exiba o valor real em vez de zerado
    if(dias && whatsRows.length){
      var whatsPorData={};
      whatsRows.forEach(function(w){whatsPorData[w.data]=(whatsPorData[w.data]||0)+(w.total||0);});
      STATE[l].dias.forEach(function(d){d.whats=whatsPorData[d.data]||0;});
    }
  }
...
function renderVendas(){
  ...
  var _hoje = new Date();
  var hojeStr = (_hoje.getDate()<10?'0':'')+_hoje.getDate()+'/'+(_hoje.getMonth()+1<10?'0':'')+(_hoje.getMonth()+1)+'/'+_hoje.getFullYear();
  var diaHojeSheet = (STATE[loja]?STATE[loja].dias:[]).find(function(d){return d.data===hojeStr;});
  var metaHoje = (diaHojeSheet && diaHojeSheet.metaDia>0) ? diaHojeSheet.metaDia : getMetaDia(loja, todayDW);
